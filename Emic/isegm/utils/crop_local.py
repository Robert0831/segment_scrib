...
def getLargestCC(segmentation):
    if segmentation.sum()<10:
        return segmentation
    labels = label(segmentation) #算出連通的區域,不連通的用不同數字代表
    largestCC = labels == np.argmax(np.bincount(labels.flat)[1:])+1 #np.bincount 看每個數字的count,第一個是0 背景
    return largestCC #取出最大的那塊
