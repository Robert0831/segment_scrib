...
def get_rough_next_scrib(pred, gt, scrib, positive_click_num_array, negative_click_num_array,click_indx,pred_thresh=0.49):
    ...
    fn_mask_p = np.pad(fn_mask, ((0, 0), (1, 1), (1, 1)), 'constant').astype(np.uint8) #pad一圈0
    fp_mask_P = np.pad(fp_mask, ((0, 0), (1, 1), (1, 1)), 'constant').astype(np.uint8)
    num_points = int(click_indx+1)
    scrib = scrib.clone().detach().cpu()

    for bindx in range(fn_mask.shape[0]):
        fn_mask_dt = cv2.distanceTransform(fn_mask_p[bindx], cv2.DIST_L2, 5)[1:-1, 1:-1]
        fp_mask_dt = cv2.distanceTransform(fp_mask_P[bindx], cv2.DIST_L2, 5)[1:-1, 1:-1]
        ...
        dt = fn_mask[bindx] if is_positive else fp_mask[bindx] #(H,W)
