...
#max_num_points=24, prob_gamma=0.80, merge_objects_prob=0.15, max_num_merged_objects=2, use_hierarchy=False, first_click_center=True
class FirstPointSampler(BasePointSampler):
    ...
    def sample_object(self, sample: DSample):
        if len(sample) == 0:
            bg_mask = sample.get_background_mask() #取出background的地方 1,0
        ...
        gt_mask, pos_masks, neg_masks = self._sample_mask(sample) #1.全部mask的聯集看1,0 只有一張, 2.一張圖的所有mask轉1,0, 3.同2 negtive
        binary_gt_mask = gt_mask > 0.5 if self.soft_targets else gt_mask > 0 #T F 

        self.selected_mask = gt_mask 
        self._selected_masks = pos_masks

        neg_mask_bg = np.logical_not(binary_gt_mask) #取反的 positive
        neg_mask_border = self._get_border_mask(binary_gt_mask) #背景邊界
    ...
    def _sample_mask(self, sample: DSample):
        ...
        if len(root_obj_ids) > 1 and random.random() < self.merge_objects_prob:
            ...
            random_ids = [random.choice(root_obj_ids)] #抽一個
        ...
        for obj_id in random_ids:
            ...
            if gt_mask is None:
                ...
                gt_mask = np.maximum(gt_mask, obj_gt_mask) # 記錄所有有物件的地方
        ...
        neg_masks = [self._positive_erode(x) for x in neg_segments] #沒取 []
    ...
    def _sample_from_masks_layer(self, obj_id, sample: DSample):
        ...
        if not self.use_hierarchy:
            node_mask = sample.get_object_mask(obj_id) #取出1,0的mask
    ...
    def _sample_center_points(self, mask):
        ...
        point_indx = 0
        click = point_indices[np.random.randint(0, num_indices)].tolist() + [point_indx] #隨機選一個點  輸出 [[row,col,index]]
        return [click]
    ...
    def _get_border_mask(self, mask):
        ...
        kernel = np.ones((3, 3), np.uint8)
        expanded_mask = cv2.dilate(mask.astype(np.uint8), kernel, iterations=expand_r) #把區域膨脹
        expanded_mask[mask.astype(np.bool)] = 0 #去調原本的 剩邊界
...
class MultiPointSampler(BasePointSampler):
    ...
    def _get_border_mask(self, mask):
        expand_r = int(np.ceil(self.expand_ratio * np.sqrt(mask.sum()))) #np.ceil無條件進位
        kernel = np.ones((3, 3), np.uint8)
...
def get_point_candidates(obj_mask, k=1.7, full_prob=0.0):
    ...
    dt = cv2.distanceTransform(padded_mask.astype(np.uint8), cv2.DIST_L2, 0)[1:-1, 1:-1] #算有物體的地方與最近背景的距離,然後去掉padding
    if k > 0:
        inner_mask = dt > dt.max() / k
        return np.argwhere(inner_mask)  #給出True的位置(row,column)
    else:
        prob_map = dt.flatten()
