...
def paint_point_track(
  ...
  radius = int(round(min(height, width) * dot_size_as_fraction_of_min_edge)) #畫的點的大小
  diam = radius * 2 + 1
  quadratic_y = np.square(np.arange(diam)[:, np.newaxis] - radius - 1) #newaxis 增加維度用 .ex (4,) => (4.1)
